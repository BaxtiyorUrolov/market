...
func (s *repositoryTransactionRepo) GetList(ctx context.Context, req models.GetListRequest) (models.RepositoryTransactionsResponse, error) {
	...
	if req.Search != "" {
		countQuery += fmt.Sprintf(`WHERE quantity ILIKE '%%%s%%' or price ilike '%%%s%%'`, req.Search, req.Search)
	}
	...
	if req.Search != "" {
		query += fmt.Sprintf(` WHERE quantity ILIKE '%%%s%%' or price ilike '%%%s%%'`, req.Search, req.Search)
	}
